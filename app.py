# -*- coding: utf-8 -*-
"""
축제가 온라인 관심도(SNS 검색량)에 미치는 영향 분석
실행: streamlit run app.py
"""
import sqlite3
import numpy as np
import pandas as pd
import streamlit as st
import plotly.express as px
import plotly.graph_objects as go
from scipy import stats

st.set_page_config(page_title="축제 온라인 관심도 분석", layout="wide")

PERIOD_ORDER = ["BEFORE_3M","BEFORE_2M","BEFORE_1M","FESTIVAL","AFTER_1M","AFTER_2M","AFTER_3M"]
OFFSET_LABEL = {-3:"BEFORE_3M",-2:"BEFORE_2M",-1:"BEFORE_1M",0:"FESTIVAL",
                1:"AFTER_1M",2:"AFTER_2M",3:"AFTER_3M"}

# ── 핵심 분석 SQL ─────────────────────────────────────────────────────────────
SQL_MAIN = """
WITH fest_occ AS (
    -- 처치(treatment): 2020년 이후 실제 개최 회차
    SELECT sido, sigungu, festival_name,
           festival_year                              AS anchor_year,
           year_month                                 AS anchor_ym,
           (year_month % 100)                         AS moy,
           (year_month/100)*12 + (year_month%100 - 1) AS m_idx,
           '처치' AS kind
    FROM festival
    WHERE period = 'FESTIVAL' AND festival_year >= 2020
),
placebo_occ AS (
    -- 반사실(placebo): 같은 달 × 코로나 미개최 연도(2020·2021)
    SELECT DISTINCT f.sido, f.sigungu, f.festival_name,
           py.y                   AS anchor_year,
           py.y*100 + f.moy       AS anchor_ym,
           f.moy,
           py.y*12 + (f.moy - 1) AS m_idx,
           '반사실' AS kind
    FROM fest_occ f
    CROSS JOIN (SELECT 2020 AS y UNION ALL SELECT 2021) py
),
joined AS (
    -- 처치+반사실 anchor에 SNS 검색량 ±3개월 결합
    SELECT a.sido, a.sigungu, a.festival_name, a.kind,
           a.anchor_year, a.anchor_ym,
           (s.m_idx - a.m_idx) AS off,
           s.search_volume      AS sv
    FROM (SELECT * FROM fest_occ UNION ALL SELECT * FROM placebo_occ) a
    JOIN (SELECT sido, sigungu, search_volume,
                 (year_month/100)*12 + (year_month%100 - 1) AS m_idx
          FROM sns_mention) s
      ON s.sido = a.sido AND s.sigungu = a.sigungu
     AND s.m_idx BETWEEN a.m_idx - 3 AND a.m_idx + 3
),
anchor_metrics AS (
    -- anchor별 지표 집계 (완전 ±3개월 윈도우만 사용)
    SELECT sido, sigungu, festival_name, kind, anchor_year, anchor_ym,
           MAX(CASE WHEN off = 0               THEN sv END)  AS vol_festival,
           AVG(CASE WHEN off BETWEEN -3 AND -1 THEN sv END)  AS avg_before,
           AVG(CASE WHEN off BETWEEN  1 AND  3 THEN sv END)  AS avg_after,
           AVG(CASE WHEN off <> 0              THEN sv END)  AS baseline,
           1.0 * MAX(CASE WHEN off=0 THEN sv END)
               / AVG(CASE WHEN off<>0 THEN sv END)           AS lift,
           AVG(CASE WHEN off BETWEEN 1 AND 3 THEN sv END)
           / AVG(CASE WHEN off BETWEEN -3 AND -1 THEN sv END) AS afterglow
    FROM joined
    GROUP BY sido, sigungu, festival_name, kind, anchor_year, anchor_ym
    HAVING COUNT(CASE WHEN off BETWEEN -3 AND -1 THEN 1 END) = 3
       AND COUNT(CASE WHEN off BETWEEN  1 AND  3 THEN 1 END) = 3
       AND MAX(CASE WHEN off = 0 THEN 1 ELSE 0 END) = 1
)
-- 축제 단위 집계: 처치 vs 반사실 평균 비교
SELECT sido, sigungu, festival_name,
       COUNT(CASE WHEN kind='처치'  THEN 1 END)               AS n_editions,
       AVG(CASE WHEN kind='처치'  THEN lift      END)          AS lift,
       AVG(CASE WHEN kind='반사실' THEN lift      END)          AS plac_lift,
       AVG(CASE WHEN kind='처치'  THEN afterglow END)          AS afterglow,
       AVG(CASE WHEN kind='반사실' THEN afterglow END)          AS plac_afterglow,
       AVG(CASE WHEN kind='처치'  THEN vol_festival END)       AS vol_festival,
       AVG(CASE WHEN kind='처치'  THEN baseline   END)         AS baseline
FROM anchor_metrics
GROUP BY sido, sigungu, festival_name;
"""

# 전후 프로파일 (처치 회차, baseline=100 정규화)
SQL_PROFILE = """
WITH fest_occ AS (
    SELECT sido, sigungu, festival_name, year_month AS anchor_ym,
           (year_month/100)*12 + (year_month%100 - 1) AS m_idx
    FROM festival WHERE period='FESTIVAL' AND festival_year >= 2020
),
window_data AS (
    SELECT f.festival_name, f.anchor_ym,
           (s.m_idx - f.m_idx) AS off, s.search_volume AS sv,
           AVG(CASE WHEN s.m_idx <> f.m_idx THEN s.search_volume END)
               OVER (PARTITION BY f.festival_name, f.anchor_ym) AS baseline,
           COUNT(CASE WHEN s.m_idx < f.m_idx THEN 1 END)
               OVER (PARTITION BY f.festival_name, f.anchor_ym) AS nb,
           COUNT(CASE WHEN s.m_idx > f.m_idx THEN 1 END)
               OVER (PARTITION BY f.festival_name, f.anchor_ym) AS na
    FROM fest_occ f
    JOIN (SELECT sido, sigungu, search_volume,
                 (year_month/100)*12 + (year_month%100 - 1) AS m_idx
          FROM sns_mention) s
      ON s.sido=f.sido AND s.sigungu=f.sigungu
     AND s.m_idx BETWEEN f.m_idx-3 AND f.m_idx+3
)
SELECT festival_name, off,
       AVG(100.0 * sv / baseline) AS idx100
FROM window_data
WHERE nb=3 AND na=3
GROUP BY festival_name, off;
"""


@st.cache_data(show_spinner=False)
def load(db_path: str):
    con = sqlite3.connect(db_path)
    try:
        F = pd.read_sql(SQL_MAIN, con)
        P = pd.read_sql(SQL_PROFILE, con)
    finally:
        con.close()
    F["net_lift"]      = F.lift - F.plac_lift
    F["net_afterglow"] = F.afterglow - F.plac_afterglow
    return F, P


def wilcoxon_test(a: pd.Series, b: pd.Series):
    """Wilcoxon signed-rank test (paired). 결측 제거 후 검정."""
    a, b = np.asarray(a, float), np.asarray(b, float)
    mask = ~(np.isnan(a) | np.isnan(b))
    n = int(mask.sum())
    try:
        w = stats.wilcoxon(a[mask], b[mask])
        return float(w.statistic), float(w.pvalue), n
    except Exception:
        return np.nan, np.nan, n


# ── UI ────────────────────────────────────────────────────────────────────────
st.title("축제가 온라인 관심도에 미치는 영향")
st.caption("2020년 이후 개최 축제 전체 · 이중차분(DiD) · ±3개월 윈도우")

with st.sidebar:
    st.header("설정")
    db_path = st.text_input("SQLite DB 경로", value="festival.db")
    st.divider()
    st.markdown("""
**지표**
- **Lift** = 축제月 ÷ 전후 6개월 평균 (단기)
- **Afterglow** = 후3M ÷ 전3M (장기)
- **Net = 처치 − 반사실** (계절보정)

**선별 기준**
- Net Lift **> 0** (단기 상승)
- Net Afterglow **> 0** (장기 상승)
""")

try:
    F, P = load(db_path)
except Exception as ex:
    st.error(f"분석 실패: {ex}")
    st.stop()

V   = F.dropna(subset=["net_lift","net_afterglow"]).copy()
SEL = V[(V.net_lift > 0) & (V.net_afterglow > 0)].sort_values("net_afterglow", ascending=False).copy()

lp_stat, lp_p, ln = wilcoxon_test(V.lift,      V.plac_lift)
ap_stat, ap_p, an = wilcoxon_test(V.afterglow, V.plac_afterglow)

tab1, tab2, tab3, tab4 = st.tabs(["분석 결과", "선별 축제", "개별 상세", "방법론 · SQL"])

# ── TAB 1: 분석 결과 ──────────────────────────────────────────────────────────
with tab1:
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("분석 대상", f"{len(V)}개 축제")
    c2.metric("단기효과 p값", f"{lp_p:.4f}", "유의 ✓" if lp_p < 0.05 else "비유의")
    c3.metric("장기효과 p값", f"{ap_p:.4f}", "비유의 ✗" if ap_p >= 0.05 else "유의 ✓")
    c4.metric("단기+장기 상승 축제", f"{len(SEL)}/{len(V)}개")

    st.info(f"""
**결론**  
축제는 개최 당월 온라인 관심도를 **유의하게 끌어올린다** (단기 p={lp_p:.4f}, Net Lift 중앙 +{V.net_lift.median():.2f}).  
그러나 전체적으로 **장기 지속 근거는 없다** (장기 p={ap_p:.4f}) — 대부분 **'반짝 효과'**.  
이 중 단기·장기 모두 양(+)인 **{len(SEL)}개 축제**를 '온라인 관심도 상승 지역'으로 선별한다.
""")

    # 전체 4분면 산점도
    V["구분"] = V.apply(
        lambda r: "✅ 선별" if r.net_lift > 0 and r.net_afterglow > 0 else "미선별", axis=1)
    fig = px.scatter(V, x="net_lift", y="net_afterglow", text="festival_name",
                     color="구분",
                     color_discrete_map={"✅ 선별":"#2E86DE","미선별":"#D5D8DC"},
                     size="vol_festival",
                     labels={"net_lift":"Net Lift (단기 순효과)",
                             "net_afterglow":"Net Afterglow (장기 순효과)"})
    fig.add_vline(x=0, line_dash="dash", line_color="gray")
    fig.add_hline(y=0, line_dash="dash", line_color="gray")
    fig.update_traces(textposition="top center", textfont_size=8)
    fig.update_layout(height=560, title="전체 축제 단기·장기 순효과 분포 (우상단 = 선별)")
    st.plotly_chart(fig, use_container_width=True)
    st.caption("Net Lift > 0 AND Net Afterglow > 0 (우상단) → 선별 / 나머지 → 미선별")

# ── TAB 2: 선별 축제 ──────────────────────────────────────────────────────────
with tab2:
    st.subheader(f"선별 축제 {len(SEL)}개 — Net Afterglow 순위")
    st.markdown("단기(Net Lift > 0)·장기(Net Afterglow > 0) 모두 계절보정 후 양(+)인 축제. "
                "Net Afterglow 기준 내림차순 정렬.")

    # 순위 막대 (Net Afterglow 기준, Net Lift 중첩)
    s = SEL.sort_values("net_afterglow")
    fig = go.Figure()
    fig.add_trace(go.Bar(
        y=s.festival_name, x=s.net_afterglow, name="Net Afterglow (장기)",
        orientation="h", marker_color="#2E86DE",
        text=s.net_afterglow.round(3), textposition="outside"))
    fig.add_trace(go.Bar(
        y=s.festival_name, x=s.net_lift, name="Net Lift (단기)",
        orientation="h", marker_color="#A9CCE3",
        text=s.net_lift.round(3), textposition="inside"))
    fig.add_vline(x=0, line_color="black")
    fig.update_layout(barmode="group", height=520,
                      xaxis_title="순효과 크기",
                      legend=dict(orientation="h"))
    st.plotly_chart(fig, use_container_width=True)

    # 상세 테이블
    display_cols = {
        "festival_name":"축제명","sido":"시도","sigungu":"시군구",
        "n_editions":"분석회차","net_lift":"Net Lift","net_afterglow":"Net Afterglow",
        "lift":"처치 Lift","plac_lift":"반사실 Lift",
        "afterglow":"처치 Afterglow","plac_afterglow":"반사실 Afterglow"
    }
    st.dataframe(
        SEL[list(display_cols)].rename(columns=display_cols).round(3),
        use_container_width=True, hide_index=True)

# ── TAB 3: 개별 축제 상세 ────────────────────────────────────────────────────
with tab3:
    all_fests = SEL.festival_name.tolist() + \
                V[~V.festival_name.isin(SEL.festival_name)].sort_values(
                    "net_afterglow", ascending=False).festival_name.tolist()
    pick = st.selectbox("축제 선택", all_fests)
    row  = V[V.festival_name == pick].iloc[0]
    is_sel = pick in SEL.festival_name.values

    st.markdown(f"**{'✅ 선별' if is_sel else '⬜ 미선별'}** — "
                f"Net Lift **{row.net_lift:+.3f}** / Net Afterglow **{row.net_afterglow:+.3f}**")

    prof = P[P.festival_name == pick].set_index("off")["idx100"].to_dict()
    fig  = go.Figure()
    fig.add_trace(go.Scatter(
        x=[OFFSET_LABEL[o] for o in range(-3,4)],
        y=[prof.get(o) for o in range(-3,4)],
        mode="lines+markers", line=dict(width=3, color="#2E86DE")))
    fig.add_hline(y=100, line_dash="dash", line_color="gray", annotation_text="기준선 100")
    fig.update_layout(
        title=f"{pick} — 전후 검색량 프로파일 (기준선=100, 회차 평균)",
        xaxis=dict(categoryorder="array", categoryarray=PERIOD_ORDER),
        yaxis_title="정규화 검색량", height=400)
    st.plotly_chart(fig, use_container_width=True)

    c1, c2, c3, c4 = st.columns(4)
    c1.metric("처치 Lift",      f"{row.lift:.3f}", help="개최 연도 단기 지표")
    c2.metric("반사실 Lift",     f"{row.plac_lift:.3f}", help="미개최 연도 같은 달")
    c3.metric("처치 Afterglow", f"{row.afterglow:.3f}", help="개최 연도 장기 지표")
    c4.metric("반사실 Afterglow",f"{row.plac_afterglow:.3f}")

# ── TAB 4: 방법론 · SQL ───────────────────────────────────────────────────────
with tab4:
    st.markdown(f"""
## 분석 목적
축제 개최가 해당 지역의 온라인 관심도(SNS 검색량)를
(1) **단기적으로 끌어올리는지**, (2) **개최 이후에도 지속되는지** 검증한다.

---

## 데이터
- **festival**: 2020년 이후 개최 축제별 `period` 행(BEFORE_3M ~ AFTER_3M)
- **sns_mention**: 42개 시군 × 72개월(2020.01–2025.12) SNS 검색량 패널

---

## 분석 설계 — 이중차분(DiD)

### 왜 단순 비교가 안 되나
대부분 매년 같은 달에 열리는 **계절 축제**라,  
"축제月 vs ±3개월" 비교만으로는 *축제 효과*와 *그 달의 자연 검색 수요(계절성)*를 분리할 수 없다.

### 해결: 코로나 자연실험
44개 축제가 **2020·2021년에는 코로나로 전부 미개최**였고, SNS 데이터는 2020.01부터 존재한다.  
→ 같은 시군구·같은 달의 2020/2021 값 = **"축제 없던 같은 시즌"** = 반사실(counterfactual)

| | 개최 연도 | 미개최 연도(2020·2021) |
|---|---|---|
| 관측 | **처치(treatment)** | **반사실(placebo)** |
| 지표 | Lift, Afterglow | 반사실 Lift, 반사실 Afterglow |
| **순효과** | **Net = 처치 − 반사실** | ← 계절성 차감됨 |

---

## 지표 정의 (±3개월 윈도우 유지)

| 지표 | 정의 | 의미 |
|---|---|---|
| **Lift** | 축제月 검색량 ÷ 전후6개월 평균 | 단기 관심도 스파이크 |
| **Afterglow** | 후3개월 평균 ÷ 전3개월 평균 | 장기 관심도 지속 |
| **Net Lift** | 처치 Lift − 반사실 Lift | 계절보정 단기 순효과 |
| **Net Afterglow** | 처치 Afterglow − 반사실 Afterglow | 계절보정 장기 순효과 |

월 인덱스 = `연 × 12 + (월 − 1)` 로 계산해 연도 경계를 정확히 처리한다.

---

## 통계 기법

### 분석 단위: 축제 단위 집계 (Pseudoreplication 방지)
같은 축제의 여러 회차는 서로 독립이 아니다.  
→ 처치/반사실 지표를 **축제별로 평균**해 독립표본 **n = {len(V)}개**로 검정.

### Wilcoxon Signed-Rank Test (비모수 paired 검정)
- **H₀**: 처치 지표 = 반사실 지표 (순효과 = 0)  
- **왜 비모수?** n이 작고 검색량의 정규성을 보장할 수 없음. 순위 기반 검정이 이상치에 강건.  
- **왜 paired?** 같은 축제·같은 달의 처치와 반사실을 짝지어 비교하므로 개체 내 통제(within-unit control) 가능.
- 단기: W = {lp_stat:.1f}, **p = {lp_p:.4f}** → {"**유의** (H₀ 기각)" if lp_p<0.05 else "비유의"}  
- 장기: W = {ap_stat:.1f}, **p = {ap_p:.4f}** → {"**유의** (H₀ 기각)" if ap_p<0.05 else "**비유의** (H₀ 기각 불가)"}

### 효과크기 병기
p값의 한계(표본 크기 의존성)를 보완하기 위해 **Net 중앙값**과 **양(+) 축제 비율**을 함께 보고한다.
- Net Lift 중앙: **+{V.net_lift.median():.3f}** / 양(+): {int((V.net_lift>0).sum())}/{len(V)}개
- Net Afterglow 중앙: **{V.net_afterglow.median():+.3f}** / 양(+): {int((V.net_afterglow>0).sum())}/{len(V)}개

### 선별 기준 (기준B)
Net Lift > 0 **AND** Net Afterglow > 0 → **{len(SEL)}/{len(V)}개** 선별  
→ 이 지역들이 방문자·소비 데이터 종합분석의 입력값이 된다.

---

## 한계
1. **반사실 = 코로나 시기**: 팬데믹 이동 위축이 대조군에 섞여 순효과가 다소 과대평가될 수 있음
2. **시군구 단위 SNS**: 한 지역에 여러 축제가 있으면 신호가 혼입됨(논산·하동 등)
3. **대리지표**: 검색량 ≠ 실제 방문·소비. 관심도의 근사치로 해석.
4. **연관 ≠ 인과**: 외부 사건(뉴스·바이럴) 완전 통제 불가
""")

    st.markdown("---")
    st.markdown("### 핵심 분석 SQL")
    st.markdown("처치·반사실 결합, ±3개월 윈도우 집계, 축제 단위 평균 비교를 단일 쿼리로 처리한다.")
    st.code(SQL_MAIN, language="sql")

    st.markdown("### 전후 프로파일 SQL")
    st.markdown("처치 회차를 각자의 기준선(baseline=100)으로 정규화해 offset별 평균 프로파일을 산출한다.")
    st.code(SQL_PROFILE, language="sql")
